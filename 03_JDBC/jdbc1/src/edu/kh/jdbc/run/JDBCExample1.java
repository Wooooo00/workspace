...
public class JDBCExample1 {
	public static void main(String[] args) {
		// JDBC란? 
		// JAVA에서 DB에 접근할 수 있게 해주는 JAVA 제공 코드(interface, class)
		// java.sql 패키지에 존재
		
		// EMPLOYEE 테이블에서 
		// 사번, 이름, 부서 코드, 직급 코드, 급여, 입사일을 조회해서
		// 자바 프로그램으로 얻어와서 콘솔에 출력하기
		
		// 1. JDBC 객체 참조 변수 선언
		
		// Connection 
		// 특정 데이터베이스와의 연결 정보를 저장한 객체
		// dBeaver에서 DB 연결을 위해 주소, 계정 정보 저장한 것과 같음 
		...
		ResultSet rs = null;
		  ...
		  try { 
			 ...
			 // Connection 객체 생성 확인(오류가 없다면 오타 없이 작성한 것)
		// 확인하고 주석처리	 System.out.println(conn);
			 
			 // 3. SQL 작성
			 // 단, 마지막 세미콜론은 제외해야 한다.
			 String sql = "SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE, SALARY, HIRE_DATE   \r\n"
			 		+ "FROM EMPLOYEE";
			 // 세미 콜론이 SQL 끝에 작성되면 "SQL 명령어가 올바르게 종료되지 않았습니다" 예외 발생
			...
			// ClassNotFoundException : 오라클 jdbc 드라이버가 없을 때 발생한다.
            // -> 경로 오타 또는 라이브러리 추가를 하지 않은 둘 중 하나의 상황에서 발생한다.			
			e.printStackTrace();

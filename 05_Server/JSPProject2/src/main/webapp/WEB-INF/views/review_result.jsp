...
<body>
   ...
   <pre>
      JSP에서 HTML 형식으로 출력하려는 JAVA 값을 쉽고 효율적으로 작성하기 위한 언어
   ...
   <h3>\${ param.name속성값} :\ escape 문자 파라미터 출력</h3>

   <pre>
   - 해당 파라미터를 얻어와 출력 
   - 같은 name 속성이 여러 개일 경우 첫 번째만 출력한다.
   - 같은 name 속성이 없는 경우 출력하지 않는다.
   </pre>
   ...
   <h3>\${paramValues.name속성값들} : 파라미터 모두 얻어오기</h3>

   <pre>
      - name 속성값이 일치하는 모든 파라미터를 얻어와 배열로 반환
      -> index를 이용해서 요소 하나하나 접근이 가능하다
   </pre>

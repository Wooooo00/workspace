...
<body>
   ...
   <pre>
      ...
      - 중요!! session은 브라우저마다 하나씩 생성된다(새 탭, 새 창은 해당 X)

      - 유지범위
      - 사이트 접속부터 브라우저 |(or) 세션 만료
      ...
      <h4>5. 내장 객체의 우선 순위 : </h4>
      page > request > session > application
      - 범위가 좁을 수록 높은 우선 순위를 가진다.
   </pre>
   ...
   <% // page scope 
    ...
    <h2>\${key} : 4종류의 scope 객체를 범위가 좁은 순서대로 탐색해서 key가 일치하는 속성이 존재하면 출력</h2>
    <h2>${str}</h2>

    <hr>

    <h2>\${XXXScope.key} : XXX 자리에 scope를 작성하면 
    해당 scope에서 key가 일치하는 속성을 찾아 출력</h2>

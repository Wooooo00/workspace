...
<body>
   ...
   <pre>
      ...
      - 속성은 test만 존재
      ** test 속성값 작성시 주의사항
      1) 속성값은 무조건 EL값으로 작성해야 한다.
            test="\${key == 123}"
      2) 속성값은 무조건 true/false가 최종 결과로 나와야 한다.

      3) 속성값을 작성하는 "" 맨 앞/뒤에는 띄어쓰기가 있으면 안 된다!!! 

         오류 예시 : test="  \${key = 123}" // 앞에 공백이 존재
         오류 예시 : test="\${key = 123}  " // 뒤에 공백이 존재
   </pre>
   ...
   <%-- 별도의 else 구문이 존재하지 않는다. --%>
   <c:if test="${price != 9000}" >
      <h5>${price}원이 아닙니다.</h5>
   </c:if>

   <%-- fmt:parseNumber 태그 
         - 값을 숫자로 변환하여 새로운 변수를 생성해서 대입
            (변수 생성은 선택 가능)

         - 속성
         1) value : 숫자로 바꾸려는 값(필수)
         2) pattern : value의 패턴을 인식하여 변환할 수 있게 한다.
   ...
   <c:if test="${parsePrice <= parseMoneymoney}">
      <h4>${menu}(을)를 먹을 수 있습니다!</h4>
   </c:if>

   <c:if test="${parsePrice > parseMoney}" >
      <h4>${menu}(을)를 먹을 수 없습니다 ㅠ</h4>
   </c:if>
   ...
   <pre>
      ...
      *** c:choose 태그 작성 주의 사항 ***
      ...
      - c:otherwise 태그 : else 역할을 태그로 아무런 속성이 없음
   </pre>

...
public class PolyService {
	...
	public void printCar() {
		...
		for(int i = 0 ; i < arr.length ; i++) {
			...
			// 배열요소 toString()을 호출해서 출력
			System.out.println( arr[i].toString() );
			// car 자료형
			// arr[i]의 자료형이 Car이라서
			
			// 바인딩 : 매서드 호출부와 실제 실행되는 매서드 코드를 묶는 것
			...
			// 동적 바인딩 
			// 메서드 호출부와 
			// 참조 변수가 가리키는 실제 객체 타입
			// 오버라이딩 된 메서드와 연결이 되는 것
		...
		for(Car c : arr) {
			...
			// 실행 중 : 오버라이딩된 포르쉐, 스파크의 메서드와 연결(동적 바인딩)
		}
		
			// 동적 바인딩은 컴파일러로 검출이 안 되기 때문에 

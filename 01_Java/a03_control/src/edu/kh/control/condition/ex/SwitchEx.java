package edu.kh.control.condition.ex;

import java.util.Scanner;

public class SwitchEx {
	
	public void ex1() { 
		Scanner sc = new Scanner(System.in);
		
		// 키보드로 정수를 입력 받아서 
		// 1: Red
		// 2: Orange
		// 3: Yellow
		// 4: Blue
		// 5: Blue
		// 1~5가 아니면 Black
		
		/* switch문 
		 * 식 하나의 결과로 많은 경우의 수를 처리할 때 사용하는 조건문
		 * 식의 결과에 맞는 case 구문이 수행됨
		 * 
		 * [작성법]
		 * 
		 * switch(식) { // 식의 결과는 정수 또는 문자열
		 * case 결과1 : 결과1의 수행 코드; break;
		 * case 결과2 : 결과2의 수행 코드; break;
		 * case 결과3 : 결과3의 수행 코드; break;
		 * default : 모든 case가 만족하지 못할 때 수행할 코드
		 * }
		*/
		
		// switch 
		System.out.print("정수 입력 : ");
		int input = sc.nextInt();
		String result;
		
		switch(input) { 
		case 1 : result = "RED"; break;
		case 2 : result = "ORANGE"; break;
		case 3 : result = "YELLOW"; break;
		case 4 : result = "GREEN"; break;
		case 5 : result = "BLUE"; break;
		
		default : result = "BLACK"; break;
		}
	}
		
			
		
				
 //-------------------------------------------
	public void ex2() { 
		// 점메추
		// 난수(Math.random() ), switch문
		// 1~5 사이의 난수를 발생시켜 
		// 일치하는 수의 메뉴를 출력 
		
		// 1~5 난수 생성
		
		// Math.random() : 0.0 <= x < 1.0
		// Math.random() * 5 : 0.0 <= x 
		int randomNumber = (int)(Math.random() * 5 + 1);
		
		String  menu = null; // null : 참조하는 것이 없다(빈칸)
		// String menu; // 선언(빈칸) 
		switch(randomNumber) {
		case 1 : menu = "제육"; break;
		case 2 : menu = "서브웨이"; break;
		case 3 : menu = "국밥"; break;
		case 4 : menu = "파스타"; break;
		case 5 : menu = "초밥"; break;
		}
	//	System.out.printf("오늘의 점심 메뉴는 [%s] 입니다.", menu);
	}
	
	// ------------------------------------------------------
	// 문자열로 메뉴를 입력 받아서 해당 메뉴의 가격을 출력
	
	// [실행화면]
	// 메뉴 선택(김밥, 라면, 떡볶이) : 김밥 
	// 메뉴를 선택해주세요
	public void ex3() { 
		Scanner sc = new Scanner(System.in);
		
		System.out.print("메뉴 선택(김밥, 라면, 떡볶이) : ");
		String menu = sc.next(); // 단어 입력
	
		int price; // 가격 저장용 변수 선언 
		
		switch(menu) {
		
		case "김밥" : price = 3000; break;
		case "라면" : price = 2500; break;
		case "떡볶이" : price = 4000; break;
		default : price = -1; // 잘못 입력되었다는 신호
		}
		
		if(price != -1) { // 정상 입력한 경우
		System.out.printf("%s의 가격은 %d 입니다.", menu, price);
	
		}else { // 잘못 입력한 경우
			System.out.println("존재하지 않는 메뉴입니다.");
		}
	}
		
		//  [실행 화면]
		// 정수 1 입력 : 5
		// 연산자 : + 
		// 정수2 입력 : 4
		// 5 + 4 = 9
 	
		//
	
	
	public void ex4() {
		Scanner sc = new Scanner(System.in);
		
		System.out.print("정수1 입력 : ");
		int num1 = sc.nextInt();
		
		System.out.print("연산자 : ");
		String op = sc.next();
		
		System.out.print("정수2 입력 : ");
		int num2 = sc.nextInt();
		
		switch(op) { 
		// System.out.printf(%d %s %d = %d, num1, op, num2, num1 + num2); break;
		case "+" : System.out.print(num1 + num2); break;
		case "-" : System.out.print(num1 - num2); break;
		case "*" : System.out.print(num1 * num2); break;
		case "/" : System.out.print(num1 / num2); break;
		case "%" : System.out.print(num1 / num2); break;
		
		default : System.out.println("존재하지 않는 연산자입니다.");
		
		
		
		}
		
		
		// --------------------------------------------------------------------
		/*
		 * int result = 0; 계산 결과 저장용 변수 선언 
		 * 				   초기화를 했기 때문에 switch문에서 값이 대입되지 않아도 이후 result 출력해도 문제 발생 x. 
		 * 
		 * switch(op) {
		 * case "+" : result = num1 + num2; break;
		 * 
		 * boolean flag = false; 신호용 변수 (상태를 기록하고 조건문에 사용할 변수)
		 * false 일 때: 연산자를 정상 입력
		 *
		 * true일 때 : 연산자를 잘못 입력
		 * 
		 * default : flag = true; // 연산자를 잘못 입력 -> flag를 true로 변경.
		 * flag == false 와 !flag는 같은 결과
		 * 
		 * 
		 * 
		 */
	
		
		
		
	
	
	
		
		
		// ---------------------------------------------
	    // switch 예제 5 break의 역할
	public void ex5() {
	}
	
	Scanner sc = new Scanner(System.in);
		System.out.print("달(월) 입력");
		int = sc.nextInt();
	
		
		
		switch(month) {
		
	}
	// switch문을 이용한 계절 판별기
		// break가 없는 경우
		// break를 만나거나 switch가 끝날 때까지 다음 case를 연달아 실행
		
		
		
		
		
		
		
		
		
		

		
		

	
	
				












	}









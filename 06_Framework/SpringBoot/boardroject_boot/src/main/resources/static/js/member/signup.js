 ...
 */
 ...
 // 이메일 유효성 검사에 사용할 요소 모두 얻어 오기
 const memberEmail = document.getElementById("memberEmail");
 const emailMessage = document.getElementById("emailMessage");

 // 이메일이 입력(input) 될 때마다 유효성 검사 실행
 memberEmail.addEventListener("input", () => {
   ...
   if(memberEmail.value.trim().length == 0){
      ...
      // 클래스 제거하여 글자색을 검은색으로 만들기
      emailMessage.classList.remove("confirm", "error");

      // 유효하지 않으니 checkObj의 memberEmail 값을 false로 변경
...
authKey.addEventListener("input", () => {
   if(authKey.value.trim().length == 0) {
      ...
      authKeyMessage.classList("confirm", "error");
      checkObj.authKey = false;
      return;
   }

// 입력 했으나 입력한 인증 번호가 6글자인 경우 (유효한 경우)
// -> 유효한 인증번호 형식입니다(with 초록색 글자)
// checkObj.authKey = true;


if(authKey.value.trim().length == 6) {
   authKeyMessage.innerText = "유효한 인증번호입니다.";
...
// 입력 했으나 입력한 인증 번호가 6미만인 경우 (유효하지 않은 경우)
// 유효하지 않은 인증번호 형식입니다(with 빨간색 글자)
// checkObj.authKey = false;
else {
   authKeyMessage.innerText = "유효하지 않은 인증번호입니다.";

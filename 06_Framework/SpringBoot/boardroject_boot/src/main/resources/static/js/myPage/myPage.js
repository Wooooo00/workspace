...
// 1차 이미지 value/file이 없어지게 되어서 제출 시 파일이 서버로 전달 되지 않는다
...
// 프로필 이미지가 
// -1 : 변경되지 않았을 때
// 0 : 있었는데 없어졌을 때 -> x 버튼을 클릭했을 때
// 1 : 새로운 이미지가 선택되었을 때 (없다가 있을 때, 다른 이미지로 변경 되었을 때)

let statusCheck = -1;

// input type ="file" 태그의 값이 변경 되었을 때, 변경된 상태를 백업할 변수, 백업용 인풋
// 요소.cloneNode(true/false) : 요소 복제(true라면 하위 요소도 복제) 

let backupInput; 

if(imageInput != null) { // imageInput이 존재할 때

    // 프로필 이미지 선택 시 수행할 함수
    const changeImageFn = e => {

        console.log(e.target); 
        console.log(e.target.value); // 업로드 파일 경로 fakepath
        ...
        const maxSize = 1024 * 1024; // 1MB (byte 단위) 1024 
        ...
        reader.onload = e => {
            ...
            statusCheck = 1; // 변경 되었을 때(새로운 이미지 선택)
    ...
    // 1) 미리보기를 기본 이미지로 변경
    // 2) input 태그에 value 값을 빈칸으로 변경해야 한다 
    ...
    profileFrm.addEventListener("submit", e => {
        ...
        // 1) 로그인한 회원의 프로필이 있는 상태 -> 없는 상태

        if(loginMemberProfileImg != null && statusCheck == 0) flag = false;
        
        // 2) 로그인한 회원의 프로필이 없는 상태 -> 있는 상태  

        if(loginMemberProfileImg == null  && statusCheck == 1) flag = false;

        // 3) 로그인한 회원의 프로필이 있는 상태 -> 변경 

        if(loginMemberProfileImg != null  && statusCheck == 1) flag = false;

        if(flag) {
            e.preventDefault(); // 이벤트 대상인 form 태그 제출 이벤트 막기(제거)

package edu.kh.project.admin.controller;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseBody;

import edu.kh.project.admin.model.service.AjaxService;
import edu.kh.project.member.model.dto.Member;

@Controller
@RequestMapping("ajax")
public class AjaxController {

	@Autowired
	private AjaxService service;
	
	
	/** 회원 번호가 일치하는 회원 이메일 조회
	 * @return
	 */
	@ResponseBody
	@GetMapping("selectMemberNo")
	public String selectMemberNo(int memberNo) {
		
		String email = service.selectMemberNo(memberNo);
		
		// 동기식으로 응답할 때
		// 1) 모델 객체를 이용하여 
		// 모델 객체에 데이터 세팅
		// 2) 지정된 html로 forward
		// 3) 갱신된 화면에 데이터를 출력
		
		// ------------------------------------------------
		
		// 비동기 방법으로 응답하는 방법
		// -비동기 통신은 페이지 갱신(화면 새로 만들기)가 목적이 아닌
		// 단순히 요청에 맞는 응답 데이터만 주고 받는 것이 목적 
		
		return email;
		// 문제 발생
		// 이메일을 반환 하려는데 
		// Controller에서 반환된 값을 ViewResolver에 의해
		// 자동으로 /templates/반환값.html로 forward를 수행하는 문제 발생
		
		// 해결 방법
		// 컨트롤러의 메서드 반환값
		// forward를 위한 경로가 아닌
		// 비동기 요청한 곳으로 돌아가야 하는 값 자체임을 명시하는 
		// 어노테이션 @ResponseBody를 메서드 위에 추가한다. 
	}
	
	/** 이메일로 전화번호 조회
	 * @param inputEmail
	 * @return memberTel
	 * 
	 */
	
	@GetMapping("selectEmail")
	@ResponseBody // forward 아닌 값 자체 반환(비동기 통신)
	public String selectEmail(String inputEmail) {
		
		return service.selectEmail(inputEmail);
	}
	
	@GetMapping(value="selectMember", produces = "application/json; charset=UTF-8")
	@ResponseBody
	public Member selectMember(int no) {
		
		// Java 객체는 JS에서 사용 불가능
		// Java 객체 -> JS 객체 변환 작업 필요 !!
		// java 객체를 JSON으로 변환 
		//	JSON JS 객체 모양 문자열
		// 		-> HttpMessageConverter가 변환
		
		// JSON 문자열 형태의 데이터를 JS로 전달(응답)
		// 	+ JSON이니까 JS 객체로 변환해서 사용해라는 구문을 첨부
		//		@XXXMapping() 구문에 
		// 			produces="application/json; charset=UTF-8" 추가
			
		
		// 3) JS에서 자동으로 JS 객체로 변환되어 사용 가능
		
		
		return service.selectMember(no);
		
	}
	
	// 문자열을 입력 받아 일부라도 일치하는 모든 이메일 조회
	/**
	 * @param keyword
	 * @return emailList
	 */
	@GetMapping(value="selectEmailList", produces = "application/json; charset=UTF-8")
	@ResponseBody
	public List<String> selectEmailList(String keyword){
		
		return service.selectEmailList(keyword);
	}
	
	
}

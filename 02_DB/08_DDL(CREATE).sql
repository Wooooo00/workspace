CREATE TABLE USER_TEST(
	...
	GENDER CHAR(3) CONSTRAINT CK_CHECK CHECK(GENDER IN ('남', '여')) ,
	PHONE VARCHAR2(20) , 
...
CREATE TABLE USER_TEST(
	USER_NO NUMBER CONSTRAINT PK_USER_TEST PRIMARY KEY,
	USER_ID VARCHAR CONSTRAINT UK_USER_ID UNIQUE,
	USER_PWD VARCHAR CONSTRAINT NN_USER_PWD NOT NULL,
	PNO 
	GENDER
	PHONE
	ADDRESS 
...
INSERT INTO USER_USED_CHECK
VALUES(2, 'user02', 'pass02', '박길동', '남', '010-1434-1678', 'parkg123@kh.or.kr');
INSERT INTO USER_USED_CHECK
VALUES(3, 'user03', 'pass03', '김길동', '여', '010-1224-5658', 'KIM1@kh.or.kr');
INSERT INTO USER_USED_CHECK
VALUES(4, 'user04', 'pass04', '정길동', '남', '010-1274-1278', 'J23@kh.or.kr');
INSERT INTO USER_USED_CHECK
VALUES(5, 'user05', 'pass05', '이길동', '남', '010-1514-7578', 'leee3@kh.or.kr');
...
-- COMMENT ON COLUMN 테이블명.컬럼명 IS '주석이름'
...
INSERT INTO MEMBER
VALUES('MEM01','123ABC','홍길동','231231-1234567', DEFAULT);
--TO_DATE('2023-08-28') 											
--SYSDATE 											
--DEFAULT									
...
/*
    ...
    제약조건 종류
    PRIMARY KEY(기본키, 행을 구분하는 값(EX 사번, 학번)) NOT NULL(컬럼에 NULL 입력 불가), UNIQUE(중복 불가),
    CHECK(지정된 값만 삽입, 수정 가능), FOREIGN KEY(다른 테이블의 기본키 또는 UNIQUE가 설정된 컬럼을 참조)
    -> 다른 테이블의 값을 가져다 씀.
...
--ORA-00001 : 무결성 제약 조건(A230724_LCW.SYS_C0013072)에 위배됩니다
--SYS_C0013063 : 
...
-- 참조(REFERENCES)된 다른 테이블의 컬럼이 제공하는 값만 사용할 수 있음
   -- 특정 값에 대한 중복을 제거할 수 있다(무결성 유지)
   -- 두 테이블이 같은 컬럼 값을 지님 => JOIN 	
...
-- USER_USED_FK 테이블에서 참조 중인 값 (GRADE_CODE = 10)
...
--NOT NULL -> 컬럼 레벨만 지정 가능
--UNIQUE -> 컬럼, 테이블 모두 지정 가능 

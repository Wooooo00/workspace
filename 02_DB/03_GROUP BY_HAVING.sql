...
-- 부서별(같은 부서끼리 그룹을 만들어서) 급여합 조회
SELECT DEPT_CODE, SUM(SALARY)
FROM EMPLOYEE
GROUP BY DEPT_CODE;

/* SELECT문 해석 순서 (꼭 외워야 할 것)
...
FROM EMPLOYEE
WHERE DEPT_CODE IN('D5','D6') -- 부서 코드가 'D5', 'D6'인 행만 남음
...
FROM EMPLOYEE 
WHERE EXTRACT(YEAR FROM HIRE_DATE) >= HIRE_DATE 
GROUP BY JOB_CODE;
...
-- 부서별 급여 평균이 3000000원 이상인 부서를 조회하여 부서코드 오름차순으로 정렬
 ...
 FROM EMPLOYEE
 GROUP BY DEPT_CODE
 HAVING AVG(SALARY) >= 3000000 -- 묶여진 그룹의 급여 평균이 300만 이상인 경우 조회 결과에 포함  
 ORDER BY DEPT_CODE;       	  -- 조회 결과에 포함(D6, D9)	
...
-- 부서코드가 'D5'거나 급여가 300만원이 초과하는 사원의 이름, 부서코드, 급여 조회(중복 포함)

...
SELECT EMP_ID, EMP_ NAME, EMP_CODE, DEPT_TITLE, 
FROM EMPLOYEE      
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID);
-- EMPLOYEE DEPT_CODE와
-- DEPARTMENT DEPT_ID는 의미가 같은 컬럼(값도 같음)이기 때문에 
...
WHERE LOCATION_ID = LOCATION_CODE;

-- 부서명이 '해외 영업 2부'인 사원의 사번, 이름, 부서명을 사번 오름차순으로 조회
-- 사번, 이름 -> EMPLOYEE 부서명 -> DEPARTMENT
SELECT EMP_ID, EMP_NAME, DEPT_ID
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE DEPT_TITLE = '해외 영업 2부'
ORDER BY EMP_ID ;
...
--RIGHT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); -- LEFT JOIN 23행, 하동운, 이오리
FULL OUTER JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); -- LEFT JOIN 23행, 하동운, 이오리
...
SELECT EMP_NAME, DEPT_CODE, DEPT_ID, DEPT_
...
--> CROSS JOIN ALL_GOLDENGATE_CONTAINER_RULES가 나타남
...
SELECT EMP_NAME, DEPT_TITLE, LOCATION_NAME, NATIONAL_NAME
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE)
